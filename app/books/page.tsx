  ...
  const books = [
    {
      ...
      image: "/book1.jpg",
      amazonUrl: "https://www.amazon.com/There-East-Wind-Blowing-Brooks/dp/B08LG7YS23/",
      rating: 5,
    ...
    },
    {
      ...
      image: "/Book2.jpg",
      amazonUrl: "https://www.amazon.com/THERE-EAST-WIND-BLOWING-RAVEN/dp/B0BRNYLY59",
      rating: 5,
      ...
      <div className="max-w-5xl mx-auto relative z-10 animate-fade-in-up">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16">
          {books.map((book, index) => (
            <div 
              key={index} 
              className="bg-slate-900/40 border border-slate-900 rounded-2xl p-6 md:p-8 flex flex-col justify-between backdrop-blur-sm transition-all duration-300 hover:-translate-y-2 hover:border-slate-800 hover:shadow-2xl hover:shadow-amber-500/5 group"
            >
              <div>
                {/* Book Cover Image */}
                <div className="relative w-44 h-64 mx-auto mb-8 bg-slate-950 rounded-lg overflow-hidden shadow-xl border border-slate-800 group-hover:scale-105 transition-transform duration-300">
                  <Image
                    src={book.image}
                    alt={book.title}
                    fill
                ...
                <div className="flex justify-between items-center text-xs font-semibold text-amber-500/80 mb-2">
                  <span className="uppercase tracking-wider">{book.subtitle}</span>
                  <span className="text-slate-500">{book.published}</span>
                </div>

                {/* Title */}
                <h3 className="text-2xl font-bold font-serif text-slate-50 mb-4">
                  {book.title}
                </h3>
